...
function parseUsgsDischarge(returnedData) {
    ...
    try {
        ...
        temp.forEach(function (d, index, array) {
           ...
           // +"1e-4";              // -> 0.0001
           // +"-12";               // -> -12：
            //1立方英尺=0.0283168立方米,max() 方法可返回两个指定的数中带有较大的值的那个数。
            var Q = Math.max(0, +d.value * 0.0283168)||null;
            //convert cfs(cubic feet per second 立方英尺/秒) to cms.
            output[index][1] = Q;
            //    a more precise way would be to obtain the actual 'noDataValue'(更精确的方法是获得实际的“noDataValue”)
...
function getDischarge(siteId, source, options) {
  ...
  //我的内部站点ID有一个两个字母前缀，添加到提供者的站点ID中，以防止两个提供者使用相同的站点ID时产生混淆。
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            //触发initialize.js中的viewModel.dataArray.subscribe..绘制折线图
